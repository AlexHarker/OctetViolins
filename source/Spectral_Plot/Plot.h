...
namespace HISSTools
{
    ...
    public:
        ...
        public:
            ...
            double xValToPos(double value) const        { return mX + valueToPosition(value, mXLo, mXHi, mW, mXLog); }
            double yValToPos(double value) const        { return mH + mY - valueToPosition(value, mYLo, mYHi, mH, mYLog); }
            double posToXVal(double position) const     { return positionToValue(position - mX, mXLo, mXHi, mW, mXLog); }
            double posToYVal(double position) const     { return positionToValue(mH + mY - position, mYLo, mYHi, mH, mYLog); }
            
            double getX() const         { return mX; }
            double getY() const         { return mY; }
            double getL() const         { return getX(); }
            double getT() const         { return getY(); }
            double getR() const         { return getX() + getWidth(); }
            double getB() const         { return getY() + getHeight(); }
            ...
            bool inX(double x) const    {  return (x > getL() && x < getR()); }
            bool inY(double y) const    {  return (y > getT() && y < getB()); }
            bool clipX(double x) const  {  return std::max(getL(), std::min(x, getR())); }
            bool clipY(double y) const  {  return std::max(getT(), std::min(y, getB())); }
        ...
        private:
            ...
            double valueToPosition(double value, double loIn, double hiIn, double length, bool logarithmic) const
            {
                if (logarithmic)
                    value = log(value);
            ...
            double positionToValue(double value, double loIn, double hiIn, double length, bool logarithmic) const
            {
    ...
    public:
        ...
        double getX() const                 { return mScale.getX(); }
        double getY() const                 { return mScale.getY(); }
        double getL() const                 { return getX(); }
        double getT() const                 { return getY(); }
        double getR() const                 { return getX() + getWidth(); }
        double getB() const                 { return getY() + getHeight(); }
        double getWidth() const             { return mScale.getWidth(); }
        double getHeight() const            { return mScale.getHeight(); }
        const Scaling& getScaling() const   { return mScale; }
