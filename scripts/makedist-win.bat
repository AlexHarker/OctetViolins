...
if %1 == 1 (echo Making OctetViolins Windows DEMO VERSION distribution ...) else (echo Making OctetViolins Windows FULL VERSION distribution ...)
...
msbuild OctetViolins.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly

echo Building 64 bit binaries...
msbuild OctetViolins.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\OctetViolins.aaxplugin\Contents\Win32\OctetViolins.aaxplugin --out .\build-win\aax\bin\OctetViolins.aaxplugin\Contents\Win32\OctetViolins.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\OctetViolins.aaxplugin\Contents\x64\OctetViolins.aaxplugin --out .\build-win\aax\bin\OctetViolins.aaxplugin\Contents\x64\OctetViolins.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\OctetViolins.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\OctetViolins.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "OctetViolins Installer" ".\installer\OctetViolins Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\OctetViolins Installer.exe" ".\installer\OctetViolins Demo Installer.exe"
REM -del ".\installer\OctetViolins Installer.exe"
REM -)
